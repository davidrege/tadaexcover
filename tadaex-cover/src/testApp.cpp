...
void testApp::setup(){	
	...
	sevomin.loadImage("sevomin.png");
	namayeshgah.loadImage("namayeshgah.png");
	salaneh.loadImage("salaneh.png");
	honar.loadImage("honar.png");
	digital.loadImage("digital.png");
    tehran.loadImage("tehran.png");
	
	for (int i = 0; i < 1200; i++){
		particle myParticle;
		myParticle.setInitialCondition(ofRandom(0,ofGetWidth()),ofRandom(0,ofGetHeight()),0,0);
		if (i > 1000) {
            myParticle.img = &sevomin;
        }
        else if (i>800) {
			myParticle.img = &namayeshgah;
			 
		} else if(i>600){
			myParticle.img = &salaneh;
		} else if (i>400) {
			myParticle.img = &honar;
		} else if (i>200) {
			myParticle.img = &digital;
		} else {
			myParticle.img = &tehran;
		}
...
void testApp::update(){
	...
	for (int i = 0; i < particles.size(); i++){
		...
		if (i>1000) {
			particles[i].addAttractionForce(float(910),float(ofGetHeight()/2) + 10, 800, 0.5);
		} else if(i > 800) {
            particles[i].addAttractionForce(float(760),float(ofGetHeight()/2), 800, 0.5);
        } else if(i>600){
			particles[i].addAttractionForce(float(660),float(ofGetHeight()/2), 800, 0.5);
		} else if(i>400){
			particles[i].addAttractionForce(float(590),float(ofGetHeight()/2) + 10, 800, 0.5);
		} else if(i>200){
			particles[i].addAttractionForce(float(445),float(ofGetHeight()/2) + 3, 800, 0.5);
		} else {
			particles[i].addAttractionForce(float(360),float(ofGetHeight()/2), 800, 0.5);
		}
